...
- Shared Merlin-style flow-page CSS lives in `merlin_flow.css` for thin login/logout/upload/update/restart pages that should not load the full menu shell.
- `merlin_legacy.css` provides a compatibility skin for remaining Padavan Bootstrap-era pages whose backend-heavy DOM should stay intact while using Merlin colors, spacing, buttons, labels, and table chrome.
...
- Migrated form/status pages: `Advanced_LAN_Content.asp`, `Advanced_DHCP_Content.asp`, `Advanced_Wireless_Content.asp`, `Advanced_Wireless2g_Content.asp`, `Advanced_WAN_Content.asp`, `Advanced_DDNS_Content.asp`, `Advanced_System_Content.asp`, `Advanced_System_Info.asp`, `Advanced_GWStaticRoute_Content.asp`, `Advanced_VirtualServer_Content.asp`, `Advanced_Exposed_Content.asp`, `Advanced_BasicFirewall_Content.asp`, `Advanced_Firewall_Content.asp`, `Advanced_Netfilter_Content.asp`, `Advanced_URLFilter_Content.asp`, `Advanced_MACFilter_Content.asp`, `Advanced_OperationMode_Content.asp`, `Advanced_FirmwareUpgrade_Content.asp`, `Advanced_SettingBackup_Content.asp`, `Advanced_APLAN_Content.asp`, `Advanced_Switch_Content.asp`, `Advanced_Services_Content.asp`, `Advanced_Tweaks_Content.asp`, `Advanced_Scripts_Content.asp`, `Advanced_Console_Content.asp`, `Advanced_InetDetect_Content.asp`, `Advanced_WOL_Content.asp`, `Advanced_IPTV_Content.asp`, `Advanced_IPv6_Content.asp`, `Advanced_WSecurity_Content.asp`, `Advanced_WSecurity2g_Content.asp`, `Advanced_ACL_Content.asp`, `Advanced_ACL2g_Content.asp`, `Advanced_WMode_Content.asp`, `Advanced_WMode2g_Content.asp`, `Advanced_WAdvanced_Content.asp`, `Advanced_WAdvanced2g_Content.asp`, `Advanced_WGuest_Content.asp`, `Advanced_WGuest2g_Content.asp`, `Main_TrafficMonitor_realtime.asp`, `Main_TrafficMonitor_last24.asp`, `Main_TrafficMonitor_daily.asp`, `vpncli.asp`, `vpnsrv.asp`, and `as.asp`. They now use the Merlin page shell and `FormTable`, but still submit the original Padavan fields to `/start_apply.htm` or use the original page-specific status/data handlers.
- Migrated legacy-compatible pages: `Advanced_AiDisk_ftp.asp`, `Advanced_AiDisk_samba.asp`, `Advanced_AiDisk_others.asp`, `Advanced_Modem_others.asp`, `Advanced_Printer_others.asp`, `Shadowsocks.asp`, `dns-forwarder.asp`, `mentohust.asp`, `scutclient.asp`, `aidisk.asp`, `Shadowsocks_log.asp`, `mentohust_log.asp`, `scutclient_log.asp`, and visible AiDisk wizard/popup pages under `aidisk/`. These retain their original DOM, iframe, tree, popup, and apply logic while loading Merlin shell CSS, `merlin_adapter.js` where the full menu is shown, and `merlin_legacy.css` for Bootstrap compatibility.
- Migrated special flow pages: `Nologin.asp`, `Logout.asp`, `Restarting.asp`, `Updating.asp`, `Uploading.asp`, `UploadDone.asp`, `UpdateError.asp`, and `UploadError.asp`. These use `merlin_flow.css` and compact `FormTitle` cards while preserving the original parent-frame progress hooks, logout request, alert behavior, and reboot redirects.
- Migrated action/transition pages: `Shadowsocks_action.asp`, `mentohust_action.asp`, `scutclient_action.asp`, `device-map/wan_action.asp`, and `device-map/safely_remove_disk.asp`. These use `merlin_flow.css` but preserve the original backend ASP hooks, parent-frame loading calls, and redirect targets.
- Non-visual ASP endpoints intentionally remain shell-less because callers expect raw JavaScript or text: `ajax_ddnscode.asp`, `aplist.asp`, `console_response.asp`, `lan_clients.asp`, `log_content.asp`, `status_eth_mib.asp`, `status_internet.asp`, `status_lanlink.asp`, `status_wanlink.asp`, `system_status_data.asp`, `vpn_clients.asp`, `wds_aplist.asp`, `wds_aplist_2g.asp`, `wol_action.asp`, and AiDisk callback endpoints such as `aidisk/create_account.asp`, `aidisk/getfolderarray.asp`, and `aidisk/switch_AiDisk_app.asp`.
...
- `Advanced_OperationMode_Content.asp` now uses lightweight CSS scenario icons for clients, router, AP, internet, and links instead of the old `bootstrap/img/wl_device/*.png` operation-mode graphics.
- `dns-forwarder.asp` and `Advanced_Printer_others.asp` have been moved from the Bootstrap compatibility shell to the standard Merlin `content`/`FormTitle`/`FormTable` structure, so they no longer depend on `merlin_legacy.css`.
- Cleaned unused Bootstrap-era prototype resources from `n56u_merlin`: Bootstrap main CSS/JS, old Bootstrap `main.css`, unused glyphicons/big-icons/AiDisk sprites, old operation-mode PNGs, and unused Bootstrap helper scripts. Runtime assets intentionally kept under `bootstrap/` are `engage.itoggle.css`/`engage.itoggle.min.js` plus `itoggle.png`/`loading_bg.png`, Highcharts, the WOL jQuery plugins, and `wl_device/*.gif` device icons used by `device-map/clients.asp` and `disk_folder_tree.js`.
...
- `vpncli.asp` / `vpnsrv.asp` / `as.asp` (Merlin shell/FormTable, original VPN client/server form fields, OpenVPN certificate hooks, ACL management, certificate generation/export handlers, AP-mode redirect, and sitemap menu data retained)
- `Nologin.asp` / `Logout.asp` / `Restarting.asp` / `Updating.asp` / `Uploading.asp` / `UploadDone.asp` / `UpdateError.asp` / `UploadError.asp` (Merlin-style flow cards via `merlin_flow.css`, original iframe/parent progress, alert, logout, and redirect flows retained)
- `Advanced_AiDisk_ftp.asp` / `Advanced_AiDisk_samba.asp` / `Advanced_AiDisk_others.asp` / `aidisk.asp` / visible `aidisk/*.asp` wizard and popup pages (Merlin shell or legacy compatibility skin, original account/share/tree/DDNS iframe flow retained)
- `Advanced_Modem_others.asp` (Merlin shell with legacy compatibility skin, original USB modem controls retained)
- `Advanced_Printer_others.asp` / `dns-forwarder.asp` (standard Merlin shell/FormTable, original USB printer and DNS forwarder controls retained)
- `Shadowsocks.asp` / `Shadowsocks_log.asp` / `mentohust.asp` / `mentohust_log.asp` / `scutclient.asp` / `scutclient_log.asp` (Merlin shell with legacy compatibility skin, original service controls, logs, and action forms retained)
- `Shadowsocks_action.asp` / `mentohust_action.asp` / `scutclient_action.asp` / `device-map/wan_action.asp` / `device-map/safely_remove_disk.asp` (Merlin-style transition cards, original backend callbacks and redirects retained)
...
- Raw JavaScript/text/data endpoints should not be wrapped in a visual shell unless their callers are updated to parse HTML.
- Remaining visual-only work should focus on deeper hand-polishing of the legacy-compatible AiDisk and third-party service pages if the compatibility skin proves too coarse in browser QA.
